...
async def gen(slug, prompt):
    try:
        ...
        if images:
            ...
            print(f"NOIMAGE {slug}", flush=True)
    except Exception as e:
        print(f"FAIL {slug}: {str(e)[:200]}", flush=True)


async def main():
    sem = asyncio.Semaphore(4)

    async def bounded(s, p):
        async with sem:
            await gen(s, p)

    await asyncio.gather(*[bounded(s, p) for s, p in PROMPTS.items()])
